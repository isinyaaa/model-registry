"""Tests for user facing model registry APIs."""

import pytest
from model_registry.core import ModelRegistryAPIClient
from model_registry.types import ModelArtifact, ModelVersion, RegisteredModel


@pytest.mark.asyncio()
async def test_insert_registered_model(mr_api: ModelRegistryAPIClient):
    registered_model = RegisteredModel("test rm")
    rm = await mr_api.upsert_registered_model(registered_model)
    assert rm.id
    assert rm.name == registered_model.name
    assert rm.external_id is None
    assert rm.description is None
    assert rm.create_time_since_epoch
    assert rm.last_update_time_since_epoch


@pytest.mark.asyncio()
async def test_update_registered_model(mr_api: ModelRegistryAPIClient):
    registered_model = RegisteredModel("updated rm")
    rm = await mr_api.upsert_registered_model(registered_model)
    last_update = rm.last_update_time_since_epoch
    rm.description = "lorem ipsum"
    rm = await mr_api.upsert_registered_model(rm)

    assert rm.description == "lorem ipsum"
    assert rm.last_update_time_since_epoch != last_update


@pytest.fixture(scope="module")
async def registered_model(mr_api: ModelRegistryAPIClient) -> RegisteredModel:
    return await mr_api.upsert_registered_model(
        RegisteredModel("registered", external_id="mr id")
    )


@pytest.mark.asyncio()
async def test_get_registered_model_by_id(
    mr_api: ModelRegistryAPIClient,
    registered_model: RegisteredModel,
):
    assert (rm := await mr_api.get_registered_model_by_id(str(registered_model.id)))
    assert rm == registered_model


@pytest.mark.asyncio()
async def test_get_registered_model_by_name(
    mr_api: ModelRegistryAPIClient,
    registered_model: RegisteredModel,
):
    assert (
        rm := await mr_api.get_registered_model_by_params(name=registered_model.name)
    )
    assert rm == registered_model


@pytest.mark.asyncio()
async def test_get_registered_model_by_external_id(
    mr_api: ModelRegistryAPIClient,
    registered_model: RegisteredModel,
):
    assert (
        rm := await mr_api.get_registered_model_by_params(
            external_id=registered_model.external_id
        )
    )
    assert rm == registered_model


@pytest.mark.asyncio()
async def test_get_registered_models(
    mr_api: ModelRegistryAPIClient, registered_model: RegisteredModel
):
    registered_model.name += "2"
    rm2 = await mr_api.upsert_registered_model(registered_model)

    rms = await mr_api.get_registered_models()
    assert [registered_model, rm2] == rms


@pytest.mark.asyncio()
async def test_insert_model_version(
    mr_api: ModelRegistryAPIClient,
    registered_model: RegisteredModel,
):
    model_version = ModelVersion("test version", "test author")
    mv = await mr_api.upsert_model_version(model_version, str(registered_model.id))
    assert mv.id
    assert mv.name == model_version.name
    assert mv.registered_model_id is None
    assert mv.external_id is None
    assert mv.description is None
    assert mv.create_time_since_epoch
    assert mv.last_update_time_since_epoch
    assert mv.author == model_version.author


@pytest.mark.asyncio()
async def test_update_model_version(
    mr_api: ModelRegistryAPIClient, registered_model: RegisteredModel
):
    model_version = ModelVersion("updated mv", "test author")
    mv = await mr_api.upsert_model_version(model_version, str(registered_model.id))
    last_update = mv.last_update_time_since_epoch
    mv.description = "lorem ipsum"
    mv = await mr_api.upsert_model_version(mv, str(registered_model.id))

    assert mv.description == "lorem ipsum"
    assert mv.last_update_time_since_epoch != last_update


@pytest.fixture(scope="module")
async def model_version(
    mr_api: ModelRegistryAPIClient, registered_model: RegisteredModel
) -> ModelVersion:
    return await mr_api.upsert_model_version(
        ModelVersion("version", "author", external_id="mv id"),
        str(registered_model.id),
    )


@pytest.mark.asyncio()
async def test_get_model_version_by_id(
    mr_api: ModelRegistryAPIClient, model_version: ModelVersion
):
    assert (mv := await mr_api.get_model_version_by_id(str(model_version.id)))
    assert mv == model_version


@pytest.mark.asyncio()
async def test_get_model_version_by_name(
    mr_api: ModelRegistryAPIClient,
    registered_model: RegisteredModel,
    model_version: ModelVersion,
):
    assert (
        mv := await mr_api.get_model_version_by_params(
            registered_model_id=str(registered_model.id), name=model_version.name
        )
    )
    assert mv == model_version


@pytest.mark.asyncio()
async def test_get_model_version_by_external_id(
    mr_api: ModelRegistryAPIClient, model_version: ModelVersion
):
    assert (
        mv := await mr_api.get_model_version_by_params(
            external_id=model_version.external_id
        )
    )
    assert mv == model_version


@pytest.mark.asyncio()
async def test_get_model_versions(
    mr_api: ModelRegistryAPIClient,
    registered_model: RegisteredModel,
    model_version: ModelVersion,
):
    model_version.name += "2"
    mv2 = await mr_api.upsert_model_version(model_version, str(registered_model.id))

    mvs = await mr_api.get_model_versions(str(registered_model.id))
    assert [model_version, mv2] == mvs


@pytest.mark.asyncio()
async def test_insert_model_artifact(
    mr_api: ModelRegistryAPIClient,
    model_version: ModelVersion,
):
    props = {
        "name": "test model",
        "uri": "test uri",
        "model_format_name": "test format",
        "model_format_version": "test version",
        "storage_key": "test key",
        "storage_path": "test path",
        "service_account_name": "test service account",
    }
    ma = await mr_api.upsert_model_artifact(
        ModelArtifact(**props), str(model_version.id)
    )
    assert ma.id
    assert ma.name == "test model"
    assert ma.uri
    assert ma.description is None
    assert ma.external_id is None
    assert ma.create_time_since_epoch
    assert ma.last_update_time_since_epoch
    assert ma.model_format_name
    assert ma.model_format_version
    assert ma.storage_key
    assert ma.storage_path
    assert ma.service_account_name


@pytest.mark.asyncio()
async def test_update_model_artifact(
    mr_api: ModelRegistryAPIClient, model_version: ModelVersion
):
    model = ModelArtifact("updated model", "uri")
    ma = await mr_api.upsert_model_artifact(model, str(model_version.id))
    last_update = ma.last_update_time_since_epoch
    ma.description = "lorem ipsum"
    ma = await mr_api.upsert_model_artifact(ma, str(model_version.id))

    assert ma.description == "lorem ipsum"
    assert ma.last_update_time_since_epoch != last_update


@pytest.fixture(scope="module")
async def model(
    mr_api: ModelRegistryAPIClient,
    model_version: ModelVersion,
) -> ModelArtifact:
    return await mr_api.upsert_model_artifact(
        ModelArtifact("model", "uri", external_id="ma id"), str(model_version.id)
    )


@pytest.mark.asyncio()
async def test_get_model_artifact_by_id(
    mr_api: ModelRegistryAPIClient, model: ModelArtifact
):
    assert (ma := await mr_api.get_model_artifact_by_id(str(model.id)))
    assert ma == model


@pytest.mark.asyncio()
async def test_get_model_artifact_by_model_version_id(
    mr_api: ModelRegistryAPIClient, model_version: ModelVersion, model: ModelArtifact
):
    assert (
        ma := await mr_api.get_model_artifact_by_params(
            model_version_id=str(model_version.id)
        )
    )
    assert ma == model


@pytest.mark.asyncio()
async def test_get_model_artifact_by_external_id(
    mr_api: ModelRegistryAPIClient, model: ModelArtifact
):
    assert (
        ma := await mr_api.get_model_artifact_by_params(external_id=model.external_id)
    )
    assert ma == model


@pytest.mark.asyncio()
async def test_get_all_model_artifacts(
    mr_api: ModelRegistryAPIClient, model_version: ModelVersion, model: ModelArtifact
):
    model.name += "2"
    ma2 = await mr_api.upsert_model_artifact(model, str(model_version.id))

    mas = await mr_api.get_model_artifacts()
    assert [model, ma2] == mas


@pytest.mark.asyncio()
async def test_get_model_artifacts_by_mv_id(
    mr_api: ModelRegistryAPIClient, model_version: ModelVersion, model: ModelArtifact
):
    model.name = "model2"
    ma2 = await mr_api.upsert_model_artifact(model, str(model_version.id))

    mas = await mr_api.get_model_artifacts(str(model_version.id))
    assert [model, ma2] == mas
